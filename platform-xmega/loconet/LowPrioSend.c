...
#include "LowPrioSend.h"
#include "ln_sw_uart.h"



static lnMsg         stLowPrioPacket;
static unsigned char ucPrio=0;



char LowPrioSendPacket(lnMsg *pstPacket, unsigned char ucPriority)
{
  if (ucPrio!=0) return -1; // failed, previous request not finished

  ucPrio = ucPriority;
  stLowPrioPacket = *pstPacket;
  return 0;
}
...
void LowPrioSendHandle(void)
{
  if (ucPrio)
  {
    if (sendLocoNetPacketTry(&stLowPrioPacket, ucPrio) == LN_DONE)
    {
      LowPrioSendCancel();  // success, finish tries to send this
    }
  }
}
