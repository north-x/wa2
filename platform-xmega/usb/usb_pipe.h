...
static inline uint8_t usb_pipe_read_byte(const USB_Pipe* p){
	...
	if (p->data->packet_offset == p->pipe->data->read_ptr[p->pipe->size]){
		ATOMIC_BLOCK(PIPE_ATOMIC){
			p->data->packet_offset = 0;
			pipe_done_read(p->pipe);
			usb_pipe_handle(p);
...
static inline uint8_t usb_pipe_read_bytes_available(const USB_Pipe* p){
	return pipe_read_ptr(p->pipe)[p->pipe->size+1];
}