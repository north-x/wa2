...
#define PIPE_H(NAME, SLOTS, SIZE, RESERVE_READ, RESERVE_WRITE)       \
	...
	extern uint8_t NAME##_buffer[(SLOTS)*(SIZE+2)]; \
...
#define PIPE_C(NAME, SLOTS, SIZE)                   \
	uint8_t NAME##_buffer[(SLOTS)*(SIZE+2)];           \
...
static inline void pipe_done_read(const Pipe* pipe){
	pipe->data->count -= 1;
	pipe->data->read_ptr += pipe->size +2;
	// Assumes buffer_size % slot_size == 0 and all accesses aligned
	if (pipe->data->read_ptr == &pipe->buffer[pipe->slots*(pipe->size+2)])
...
static inline void pipe_done_write(const Pipe* pipe){
	pipe->data->count += 1;
	pipe->data->write_ptr += pipe->size +2;
	// Assumes buffer_size % slot_size == 0 and all accesses aligned
	if (pipe->data->write_ptr == &pipe->buffer[pipe->slots*(pipe->size+2)])
